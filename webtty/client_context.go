...
func (context *clientContext) goHandleClient() {
	...
	context.app.cc.SendConnectEvent(context.code, context.peer_ip, "wetty")
	context.app.cc.SetDataCallback(context.code, "wetty", func(data []byte) error {
	...
	go func() {
		...
		defer context.app.cc.SendDisconnectEvent(context.code, context.peer_ip, "wetty")
...
func (context *clientContext) processSend() {
	...
	for {
		...
		context.app.cc.SendLogEvent(safeMessage, context.code, "wetty")
	}
}
